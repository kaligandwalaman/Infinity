...
struct subtab_element {
    float sizex, sizey, element_opacity, rect_opacity ,rect_opacity2 , text_opacity, text_opacity1,text_opacity2, text_opacity3, text_opacity4;
};

bool ImGui::SUBTAB(const char* name, bool boolean)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImRect rect(pos, ImVec2(pos.x + 90, pos.y + 90));
    ImGui::ItemSize(ImVec4(rect.Min.x, rect.Min.y, rect.Max.x, rect.Max.y + 0), style.FramePadding.y);
    if (!ImGui::ItemAdd(rect, id))
        return false;
    bool hovered, held;
    ...
    it_anim->second.text_opacity1 = ImLerp(it_anim->second.text_opacity1, (boolean ? 1.0f : menu[0]), 0.13f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity2 = ImLerp(it_anim->second.text_opacity2, (boolean ? 1.0f : menu[1]), 0.13f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity3 = ImLerp(it_anim->second.text_opacity3, (boolean ? 1.0f : menu[2]), 0.13f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity = ImLerp(it_anim->second.text_opacity, (boolean ? 0.5f : 1.0f), 0.13f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity4 = ImLerp(it_anim->second.text_opacity4, (boolean ? 0.0f : 1.0f), 0.13f * (1.0f - ImGui::GetIO().DeltaTime));
    
    window->DrawList->AddCircle(rect.GetCenter(), 35.0f, ImColor(90/255.f,90/255.f,90/255.f,it_anim->second.text_opacity4),360.0f,2.0f);
    window->DrawList->AddShadowCircle(rect.GetCenter(), 20.0f, ImColor(menu[0],menu[1],menu[2],it_anim->second.text_opacity4), 105.0f, ImVec2(0,0), 360.0f);
    window->DrawList->AddText(F48,30.0f,rect.GetCenter() - ImVec2(15,15)/*(CalcTextSize(name)/2)*40/25*/, ImColor(it_anim->second.text_opacity1, it_anim->second.text_opacity2, it_anim->second.text_opacity3, it_anim->second.text_opacity), name);
    return pressed;
    }

bool ImGui::TAB(const char* icon, const char* name, bool boolean){
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ImGuiContext& g = *GImGui;
    ...
    const ImGuiID id = window->GetID(name);
    ImVec2 pos = window->DC.CursorPos;
    const ImRect rect(pos, ImVec2(pos.x + 235, pos.y + 60));
    ImGui::ItemSize(ImVec4(rect.Min.x, rect.Min.y, rect.Max.x, rect.Max.y + 0), style.FramePadding.y);
    if (!ImGui::ItemAdd(rect, id))
        return false;
    bool hovered, held;
    ...
    it_anim->second.rect_opacity = ImLerp(it_anim->second.rect_opacity, (boolean ? 0.0f : 0.5f), 0.15f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity = ImLerp(it_anim->second.text_opacity, (boolean ? 0.8f : 1.0f), 0.07f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.rect_opacity2 = ImLerp(it_anim->second.rect_opacity2, (boolean ? 0.0f : 1.0f), 0.15f * (1.0f - ImGui::GetIO().DeltaTime));
    
    window->DrawList->AddRectFilledMultiColor(rect.Min, rect.Max, ImColor( menu[0],  menu[1], menu[2], it_anim->second.rect_opacity),ImColor( 0/255.f,  0/255.f, 0/255.f, it_anim->second.rect_opacity),ImColor( 0/255.f,  0/255.f, 0/255.f, it_anim->second.rect_opacity),ImColor( menu[0],  menu[1], menu[2], it_anim->second.rect_opacity));   
    window->DrawList->AddRectFilled(rect.Min, ImVec2(rect.Min.x + 5, rect.Max.y), ImColor( menu[0],  menu[1], menu[2], it_anim->second.rect_opacity2), 2);
    window->DrawList->AddText( F48, 30.0f, rect.Min + ImVec2(35.f,15.f), ImColor(1.0f,1.0f,1.0f,1.0f/*it_anim->second.text_opacity*/),icon);   
    window->DrawList->AddText( NULL, 25.0f, rect.Min + ImVec2(95.f,17.5f), ImColor(1.0f,1.0f,1.0f,it_anim->second.text_opacity),name);   
    return pressed;
    }
    
/*
struct IMGUI_GOD{
    float text,text1,text2,text3,rect;
};

bool ImGui::TAB_PC(const char* icon, const char* name, const ImVec2& size_arg, bool boolean)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    static std::map <ImGuiID, IMGUI_GOD> anim;
    auto it_anim = anim.find(id);
    if (it_anim == anim.end()) {
        anim.insert({ id, { 0.0f, 0.0f, 0.0f } });
        it_anim = anim.find(id);
    ...
    it_anim->second.text1 = ImLerp(it_anim->second.text1, (boolean ? 1.0f : menu[0]), 0.13f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text2 = ImLerp(it_anim->second.text2, (boolean ? 1.0f : menu[1]), 0.13f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text3 = ImLerp(it_anim->second.text3, (boolean ? 1.0f : menu[2]), 0.13f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text  = ImLerp(it_anim->second.text , (boolean ? 0.5f : 1.0f), 0.13f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.rect  = ImLerp(it_anim->second.rect , (boolean ? 0.0f : 1.0f), 0.13f * (1.0f - ImGui::GetIO().DeltaTime));
    
    
    
    window->DrawList->AddRectFilled(bb.Min, ImVec2(bb.Min.x + 20, bb.Max.y), ImColor( menu[0],  menu[1], menu[2], it_anim->second.rect), 0.0f);
    window->DrawList->AddText(F48,25.0f, bb.Min + ImVec2(40, bb.GetSize().y / 2.f - 25/2), ImColor(menu[0],  menu[1], menu[2], it_anim->second.text), icon );
    window->DrawList->AddText( bb.Min + ImVec2(80, bb.GetSize().y / 2.f - CalcTextSize(name).y / 2.f), ImColor(it_anim->second.text1, it_anim->second.text2, it_anim->second.text3, it_anim->second.text), name );
    window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, ImColor(menu[0], menu[1], menu[2], it_anim->second.rect/5), ImColor(menu[0], menu[1], menu[2], 0.0f), ImColor(menu[0], menu[1], menu[2], 0.0f), ImColor(menu[0], menu[1], menu[2], it_anim->second.rect/5));
    return pressed;                                                                                                                                                                                                                                                 
}

*/
bool ImGui::ButtonExes1(const char* icon,const char* name, bool boolean)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(name);
    const ImVec2 label_size = ImVec2(212,60);//ImGui::CalcTextSize(name);//CalcTextSize(ico, NULL, true);

   // const ImVec2 label_size = ImVec2(100,50);
    ImVec2 pos = window->DC.CursorPos;
    ...
    const ImRect rect(pos, ImVec2(pos.x + 212, pos.y + 60));
    ImGui::ItemSize(ImVec4(rect.Min.x, rect.Min.y, rect.Max.x, rect.Max.y + 0), style.FramePadding.y);
    ...
    bool hovered, held;
    bool pressed = ImGui::ButtonBehavior(rect, id, &hovered, &held, NULL);
   // const ImColor col = ImColor((held && hovered) ? ImColor(43, 43, 46, 0) : hovered ? ImColor(43, 43, 46, 0) : ImColor(43, 43, 46, 0));
    ...
    it_anim->second.rect_opacity = ImLerp(it_anim->second.rect_opacity, (boolean ? 0.0f : 0.8f), 0.09f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity1 = ImLerp(it_anim->second.text_opacity1, (boolean ? 255/255.f : menu[0]), 0.15f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity2 = ImLerp(it_anim->second.text_opacity2, (boolean ? 255/255.f : menu[1]), 0.15f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity3 = ImLerp(it_anim->second.text_opacity3, (boolean ? 255/255.f : menu[2]), 0.15f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity = ImLerp(it_anim->second.text_opacity, (boolean ? 0.5f : 1.0f), 0.15f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.sizex = ImLerp(it_anim->second.sizex, (boolean ?  0.5f : 1.0f), 0.15f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.sizey = ImLerp(it_anim->second.sizey, (boolean ?  0.0f : 1.0f), 0.15f * (1.0f - ImGui::GetIO().DeltaTime));
    
    
    window->DrawList->AddRectFilled(ImVec2(rect.Min.x,rect.Min.y ), ImVec2(rect.Max.x ,rect.Max.y ), ImColor( 52/255.f,  52/255.f, 52/255.f, it_anim->second.sizey), 5.0f);
    window->DrawList->AddRectFilled(ImVec2(rect.Min.x,rect.Min.y + 6), ImVec2(rect.Min.x + 5,rect.Max.y - 6 ), ImColor( menu[0],  menu[1], menu[2], it_anim->second.sizey), 40.0f, ImDrawFlags_RoundCornersTopRight | ImDrawFlags_RoundCornersBottomRight);
    PushFont(F48);
    window->DrawList->AddText( rect.Min + ImVec2(40 - CalcTextSize( icon ).x / 2.f, rect.GetSize( ).y / 2.f - CalcTextSize( icon ).y / 2.f ), /*ImColor(255/255.f,255/255.f,255/255.f,it_anim->second.sizex)*/ImColor(it_anim->second.text_opacity1, it_anim->second.text_opacity2, it_anim->second.text_opacity3, it_anim->second.text_opacity), icon );
    PopFont( );
    window->DrawList->AddText( rect.Min + ImVec2(60, rect.GetSize().y / 2.f - CalcTextSize(name).y / 2.f), ImColor(255/255.f,255/255.f,255/255.f,it_anim->second.sizex), name );
    
    
    
    //window->DrawList->AddText(ImVec2(rect.Min.x + 150, (rect.Min.y + rect.Max.y + 50) / 2 - label_size.y / 2), ImColor(it_anim->second.text_opacity1, it_anim->second.text_opacity2, it_anim->second.text_opacity3, it_anim->second.text_opacity), name);
    return pressed;                                                                                                                                                                                                                                                 
}
...
bool ImGui::ButtonExes3(const char* name, bool boolean)
//bool ImGui::ButtonExes(const char* label, const ImVec2& size_arg)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(name);
    const ImVec2 label_size = ImVec2(80,70);//ImGui::CalcTextSize(name);//CalcTextSize(ico, NULL, true);

   // const ImVec2 label_size = ImVec2(100,50);
    ImVec2 pos = window->DC.CursorPos;

    const ImRect rect(pos, ImVec2(pos.x + 80, pos.y + 70));
    ImGui::ItemSize(ImVec4(rect.Min.x, rect.Min.y, rect.Max.x, rect.Max.y + 0), style.FramePadding.y);
    //ImGui::PushStyleColor(ImGuiCol_Text, IM_COL32(0, 255, 0, 255));
    if (!ImGui::ItemAdd(rect, id))
        return false;
   ...
   // const ImColor col = ImColor((held && hovered) ? ImColor(43, 43, 46, 0) : hovered ? ImColor(43, 43, 46, 0) : ImColor(43, 43, 46, 0));
    ...
    it_anim->second.element_opacity = ImLerp(it_anim->second.element_opacity, (boolean ? 0.04f : 0.0f), 0.09f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.rect_opacity = ImLerp(it_anim->second.rect_opacity, (boolean ? 0.0f : 0.0f), 0.20f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity1 = ImLerp(it_anim->second.text_opacity1, (boolean ? 100/255.f : 0.0f), 0.07f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity2 = ImLerp(it_anim->second.text_opacity2, (boolean ? 100/255.f : 1.0f), 0.07f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity3 = ImLerp(it_anim->second.text_opacity3, (boolean ? 100/255.f : 0.0f), 0.07f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity = ImLerp(it_anim->second.text_opacity, (boolean ? 0.5f : 1.0f), 0.07f * (1.0f - ImGui::GetIO().DeltaTime));
    
    
  
    
    window->DrawList->AddRectFilled(rect.Min, rect.Max, ImColor( 45/255.f,  45/255.f, 45/255.f, it_anim->second.rect_opacity), 6.0f);
    window->DrawList->AddText(ImVec2(rect.Min.x + 25, (rect.Min.y + rect.Max.y + 52) / 2 - label_size.y / 2), ImColor(it_anim->second.text_opacity1, it_anim->second.text_opacity2, it_anim->second.text_opacity3, it_anim->second.text_opacity), name);
        
    return pressed;
}

bool ImGui::ButtonExes4(const char* name, bool boolean)
//bool ImGui::ButtonExes(const char* label, const ImVec2& size_arg)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
   ...
   // const ImColor col = ImColor((held && hovered) ? ImColor(43, 43, 46, 0) : hovered ? ImColor(43, 43, 46, 0) : ImColor(43, 43, 46, 0));
    ...
    it_anim->second.element_opacity = ImLerp(it_anim->second.element_opacity, (boolean ? 0.04f : 0.0f), 0.09f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.rect_opacity = ImLerp(it_anim->second.rect_opacity, (boolean ? 0.0f : 0.0f), 0.003f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity1 = ImLerp(it_anim->second.text_opacity1, (boolean ? 100/255.f : 0.0f), 0.03f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity2 = ImLerp(it_anim->second.text_opacity2, (boolean ? 100/255.f : 1.0f), 0.03f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity3 = ImLerp(it_anim->second.text_opacity3, (boolean ? 100/255.f : 0.0f), 0.03f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity = ImLerp(it_anim->second.text_opacity, (boolean ? 0.5f : 1.0f), 0.07f * (1.0f - ImGui::GetIO().DeltaTime));
    
    
  
    
    window->DrawList->AddRectFilled(rect.Min, rect.Max, ImColor( 45/255.f,  45/255.f, 45/255.f, it_anim->second.rect_opacity), 15.0f);
    window->DrawList->AddText(ImVec2(rect.Min.x + 25, (rect.Min.y + rect.Max.y + 52) / 2 - label_size.y / 2), ImColor(it_anim->second.text_opacity1, it_anim->second.text_opacity2, it_anim->second.text_opacity3, it_anim->second.text_opacity), name);
...
bool ImGui::ButtonExes5(const char* name, bool boolean)
//bool ImGui::ButtonExes(const char* label, const ImVec2& size_arg)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
...
bool ImGui::OptButtonEx(const char* ico, const ImVec2& size_arg, bool rotation, ImGuiButtonFlags flags)
{
    ...
    it_nameanim->second = ImLerp(it_nameanim->second, hovered ? 1.f : 0.5f, g.IO.DeltaTime * 7.f);
    it_nameanim->second *= style.Alpha;

    //if (rotation) ImRotateStart();

    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(menu[0], menu[1], menu[2], it_nameanim->second));
...
bool ImGui::OptButtonEx0(const char* ico, const char* name, const ImVec2& size_arg, bool rotation, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    PushFont(JAAT);
    window->DrawList->AddText( bb.Min + ImVec2(30 - CalcTextSize( ico ).x / 2.f, bb.GetSize( ).y / 2.f - CalcTextSize( ico ).y / 2.f - 5 ), ImColor(255/255.f,255/255.f,255/255.f,0.5)/*ImColor(it_anim->second.text_opacity1, it_anim->second.text_opacity2, it_anim->second.text_opacity3, it_anim->second.text_opacity)*/, ico );
    PopFont( );
    window->DrawList->AddText( bb.Min + ImVec2(50, bb.GetSize().y / 2.f - CalcTextSize(name).y / 2.f), ImColor(255/255.f,255/255.f,255/255.f,0.5f), name );
    
    IMGUI_TEST_ENGINE_ITEM_INFO(id, ico, g.LastItemData.StatusFlags);
    return pressed;
}

bool ImGui::IMGUI_GOD_CLOSE(const char* ico, const char* name, const ImVec2& size_arg, bool rotation)
{
    return OptButtonEx0(ico, name, size_arg, rotation, ImGuiButtonFlags_None);
}


struct subtab_element2{
    float sizex1, sizey1, text1, text_opacity, text_opacity1,text_opacity2, text_opacity3;
};

bool ImGui::OptButtonEx1(const char* ico, const ImVec2& size_arg, bool rotation, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_nameanim->second = ImLerp(it_nameanim->second, hovered ? 1.f : 1.0f, g.IO.DeltaTime * 0.7f);
    it_nameanim->second *= style.Alpha;

    static std::map <ImGuiID, subtab_element2> anim;
    auto it_anim = anim.find(id);
    if (it_anim == anim.end()) {
        anim.insert({ id, { 0.0f, 0.0f, 0.0f } });
        it_anim = anim.find(id);
    ...
    it_anim->second.sizex1 = ImLerp(it_anim->second.sizex1, (hovered ? 25.f : 0.0f), 0.10f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.sizey1 = ImLerp(it_anim->second.sizey1, (hovered ? 3.f : 0.0f), 0.10f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text1 = ImLerp(it_anim->second.text1, (hovered ? 21.f : 30.f), 0.10f * (1.0f - ImGui::GetIO().DeltaTime));
    
    
 RenderFrame(ImVec2(bb.Min.x + it_anim->second.sizex1, bb.Min.y + it_anim->second.sizey1), ImVec2(bb.Max.x - it_anim->second.sizex1, bb.Max.y - it_anim->second.sizey1), ImColor(0,0,0,255), true, 10.f);

 
    //window->DrawList->AddText(NULL, it_anim->second.text1, bb.GetCenter() - label_size / 2.f, ImColor(0, 0, 0, 255), ico);                 
                
    //window->DrawList->AddText(ImVec2(rect.Min.x + 25, (rect.Min.y + rect.Max.y + 52) / 2 - label_size.y / 2), ImColor(it_anim->second.text_opacity1, it_anim->second.text_opacity2, it_anim->second.text_opacity3, it_anim->second.text_opacity), name);
        
    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 1.0f, 1.0f, it_nameanim->second));
    RenderTextClipped(bb.Min , bb.Max , ico, NULL, &label_size, style.ButtonTextAlign, &bb);
    ImGui::PopStyleColor();
...
bool ImGui::OptButton1(const char* ico, const ImVec2& size_arg, bool rotation)
{
    return OptButtonEx1(ico, size_arg, rotation, ImGuiButtonFlags_None);
}

struct subtab_element1{
    float sizex, sizey, text_opacity, text_opacity1,text_opacity2, text_opacity3;
};

bool ImGui::OptButtonEx2(const char* ico, const ImVec2& size_arg, bool rotation, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    static std::map <ImGuiID, subtab_element1> anim;
    auto it_anim = anim.find(id);
    if (it_anim == anim.end()) {
        anim.insert({ id, { 0.0f, 0.0f, 0.0f } });
        it_anim = anim.find(id);
    ...
    it_anim->second.sizex = ImLerp(it_anim->second.sizex, (hovered ? 45.f : 0.0f), 0.10f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.sizey = ImLerp(it_anim->second.sizey, (hovered ? 3.f : 0.0f), 0.10f * (1.0f - ImGui::GetIO().DeltaTime));
    
    
 RenderFrame(ImVec2(bb.Min.x + it_anim->second.sizex, bb.Min.y + it_anim->second.sizey), ImVec2(bb.Max.x - it_anim->second.sizex, bb.Max.y - it_anim->second.sizey), ImColor(202,222,252,255), true, 10.f);

    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.0f, 0.0f, 0.0f, it_nameanim->second));
    RenderTextClipped(bb.Min , bb.Max , ico, NULL, &label_size, style.ButtonTextAlign, &bb);
    ImGui::PopStyleColor();
...
bool ImGui::OptButton2(const char* ico, const ImVec2& size_arg, bool rotation)
{
    return OptButtonEx2(ico, size_arg, rotation, ImGuiButtonFlags_None);
}
...
bool ImGui::Tab(bool selected, const char* icon, const char* label, const char* hint,  const ImVec2& size_arg)
{
    ...
    ImGui::PushFont(F48); ImGui::GetWindowDrawList()->AddText(image_bb.Min, GetColorU32(it_anim->second.image), icon);ImGui::PopFont();
...
struct checkbox_animation {
    float animation,clr1,clr2,clr3,lodaa,lodaa1,lodaa2, element_opacity, rect_opacity, text_opacity, text_opacity1,text_opacity2, text_opacity3;
...
bool ImGui::Checkbox(const char* label, bool* v){
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = GetWindowWidth() - 40;
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect jaat(pos + ImVec2(20,-3), pos + ImVec2(w,label_size.y +3+ style.FramePadding.y * 2.0f));
    
    ItemSize(jaat, 0.f);
    if (!ItemAdd(jaat, id)) return false;
    bool hovered, held, pressed = ButtonBehavior(jaat, id, &hovered, &held);
    if (IsItemClicked())
    {
        *v = !(*v);
        MarkItemEdited(id);
    ...
    if (it_anim == anim.end())
    {
        anim.insert({ id, checkbox_animation() });
        it_anim = anim.find(id);
    }
    it_anim->second.animation = ImLerp(it_anim->second.animation, *v ? 1.0f : 0.0f, 0.24f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity = ImLerp(it_anim->second.text_opacity, *v ? 1.0f : 0.5f, 0.45f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.clr1 = ImLerp(it_anim->second.clr1, *v ? menu[0] : 90/255.0f, 0.17f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.clr2 = ImLerp(it_anim->second.clr2, *v ? menu[1] : 90/255.0f, 0.17f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.clr3 = ImLerp(it_anim->second.clr3, *v ? menu[2] : 90/255.0f, 0.17f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.lodaa = ImLerp(it_anim->second.lodaa, *v ? 1.0f : 0.5f, 0.17f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.lodaa1 = ImLerp(it_anim->second.lodaa1, *v ? 0.4f : 0.0f, 0.07f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.lodaa2 = ImLerp(it_anim->second.lodaa2, *v ? 1.0f : 90/255.0f, 0.07f * (1.0f - ImGui::GetIO().DeltaTime));
    
    
    
    window->DrawList->AddText(NULL,25.0f,ImVec2(jaat.Min.x, jaat.GetCenter().y - CalcTextSize(label).y/2),ImColor(1.0f,1.0f,1.0f,it_anim->second.text_opacity),label);
    window->DrawList->AddRectFilled(ImVec2(jaat.Max.x - 60, jaat.Min.y + 2), ImVec2(jaat.Max.x, jaat.Max.y-2), ImColor(menu[0], menu[1], menu[2], it_anim->second.lodaa1), 50.f,ImDrawFlags_RoundCornersAll); 
    window->DrawList->AddRect(ImVec2(jaat.Max.x - 60, jaat.Min.y + 2), ImVec2(jaat.Max.x, jaat.Max.y-2), ImColor(it_anim->second.clr1, it_anim->second.clr2, it_anim->second.clr3, it_anim->second.lodaa), 50.f,ImDrawFlags_RoundCornersAll,3.0f); 
    window->DrawList->AddCircleFilled(ImVec2(jaat.Max.x - 60 + 16 + 28.5 * it_anim->second.animation, jaat.GetCenter().y ), 10.5f,ImColor(it_anim->second.lodaa2, it_anim->second.lodaa2, it_anim->second.lodaa2, 1.0f), 360);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos + ImVec2(20, -7), window->DC.CursorPos + ImVec2(w, 25 + 7));
    const ImRect total_bb(bb.Min - ImVec2(0,2), bb.Max + ImVec2(label_size.x, 2));
    ItemSize(total_bb, 0);
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, popup_open ? c::accent_text_color : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 2.f);
    it_anim->second.combo = ImLerp(it_anim->second.combo, popup_open ? c::accent_color : hovered ? c::accent_low_color : c::text::text, g.IO.DeltaTime * 2.f);
    it_anim->second.outline = ImLerp(it_anim->second.outline, popup_open ? 1.0f : 0.0f/*c::accent_color : hovered ? c::accent_low_color : c::text::text*/, g.IO.DeltaTime * 2.f);
    it_anim->second.Rotate = ImLerp(it_anim->second.Rotate, popup_open ? -1.f : 1.f, g.IO.DeltaTime * 12.f);

      //  const float value_x2 = ImMax(bb.Min.x, bb.Max.x - arrow_size);

        GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, ImColor(24,24,24,255), 4.0f, popup_open ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersTop);
        GetWindowDrawList()->AddRect(bb.Min, bb.Max,ImColor(90,90,90) , 4.0f,ImDrawFlags_RoundCornersAll,2.0f);        
        GetWindowDrawList()->AddRect(bb.Min, bb.Max,ImColor(menu[0],menu[1],menu[2],it_anim->second.outline) , 4.0f,ImDrawFlags_RoundCornersAll,2.0f);
        GetWindowDrawList()->AddText(NULL,25.0f,bb.Min + ImVec2(17,7),ImColor(1.0f,1.0f,1.0f,popup_open ? 1.0f : 0.5f),preview_value);
        ImRotateStart();
        GetWindowDrawList()->AddText(JAAT, 25.f, ImVec2( bb.Max.x - 37 - 25/2, bb.GetCenter().y - 25/2 - 2), GetColorU32(it_anim->second.text), "P");                       
        ImRotateEnd(1.57f * it_anim->second.Rotate);
		
        /*
        PushStyleColor(ImGuiCol_Text, GetColorU32(c::accent_text_low_color));
        RenderTextClipped(bb.Min + ImVec2(17, 7), ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);
        PopStyleColor();
        */
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive)
        {
            ...
            SetNextWindowPos(pos + ImVec2(0, 6));
        }

    ImGuiWindowFlags window_flags = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_Popup | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoMove;
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(17, 9));

    PushStyleVar(ImGuiStyleVar_PopupRounding, 4.0f/*c::combo::rounding*/);

    PushStyleColor(ImGuiCol_PopupBg, ImVec4(24/255.0f,24/255.0f,24/255.0f,1.0f));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImGuiID id = window->GetID(label);
    const float w = GetContentRegionMax().x - style.WindowPadding.x;
    const float w1 = GetContentRegionMax().x - 20.0f;
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(20,- 2), window->DC.CursorPos + ImVec2(w1, label_size.y + 4 + 25.f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max /*+ ImVec2(label_size.x > 0.0f ? label_size.x : 0.0f, 0.0f)*/);

    const bool temp_input_allowed = (flags & ImGuiSliderFlags_NoInput) == 0;
    ItemSize(ImRect(total_bb.Min, frame_bb.Max + ImVec2(0, 2.f)));
    if (!ItemAdd(total_bb, id, &frame_bb, temp_input_allowed ? ImGuiItemFlags_Inputable : 0))
        return false;
    ...
    it_anim->second.circle_anim = ImLerp(it_anim->second.circle_anim, IsItemActive() ? 8.f : 10.f, g.IO.DeltaTime * 6.f);
    it_anim->second.circle = ImLerp(it_anim->second.circle, IsItemActive() ? c::accent_color : c::slider::circle_inactive, g.IO.DeltaTime * 6.f);
    ...
    it_anim->second.cl1 = ImLerp(it_anim->second.cl1, IsItemActive() ? menu[0] : 43/255.f, g.IO.DeltaTime * 6.f);
    it_anim->second.cl2 = ImLerp(it_anim->second.cl2, IsItemActive() ? menu[1] : 48/255.f, g.IO.DeltaTime * 6.f);
    it_anim->second.cl3 = ImLerp(it_anim->second.cl3, IsItemActive() ? menu[2] : 54/355.f, g.IO.DeltaTime * 6.f);
                                                                        
    GetWindowDrawList()->AddRectFilled(ImVec2(frame_bb.Min.x , frame_bb.Max.y - 18), ImVec2(frame_bb.Max.x ,frame_bb.Max.y - 12), GetColorU32(c::slider::background), 100.f);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        GetWindowDrawList()->AddShadowRect(ImVec2(frame_bb.Min.x , frame_bb.Max.y - 18), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 3, frame_bb.Max.y - 12), ImColor(menu[0], menu[1], menu[2], 1.0f), 20.f, ImVec2(0, 0), 100.f);
        GetWindowDrawList()->AddRectFilled(ImVec2(frame_bb.Min.x , frame_bb.Max.y - 18), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 3, frame_bb.Max.y - 12), ImColor(menu[0], menu[1], menu[2], 1.0f), 100.f);
        GetWindowDrawList()->AddCircleFilled(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 3, frame_bb.Max.y - 15), it_anim->second.circle_anim, ImColor(it_anim->second.cl1, it_anim->second.cl2, it_anim->second.cl3, 1.0f), 100.f);
    }
    GetWindowDrawList()->AddText( ImVec2(frame_bb.Max.x - CalcTextSize( value_buf ).x, frame_bb.Min.y + 2), GetColorU32(it_anim->second.text), value_buf, value_buf_end );
    GetWindowDrawList()->AddText(ImVec2(frame_bb.Min.x , frame_bb.Min.y + 2), GetColorU32(it_anim->second.text), label);
    return value_changed;
}
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton(label/*"##ColorButton"*/, col_v4, flags))
        {
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const float default_size = GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos;
    const float width = GetContentRegionMax().x - 30;
    const ImRect rect(pos + ImVec2(20,-5), pos + ImVec2(width, 25 + 7));
    const ImRect clickable(pos + ImVec2(width - 45, 0), pos + ImVec2(width, 25));
    ...
    RenderColorRectWithAlphaCheckerboard(window->DrawList, clickable.Min, clickable.Max + ImVec2(0, 0), GetColorU32(col_rgb), 5, ImVec2(0.f, 0.f), 5);
    GetWindowDrawList()->AddShadowRect(clickable.Min, clickable.Max + ImVec2(0, 0), GetColorU32(col_rgb), 80, ImVec2(0.f, 0.f), 5, 16);
    GetWindowDrawList()->AddText(NULL, 25.0f, ImVec2(rect.Min.x, rect.Min.y + 5), ImColor(1.0f, 1.0f, 1.0f, 0.5f),desc_id);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_anim->second.cl1 = ImLerp(it_anim->second.cl1, selected ? menu[0] : 1.0f , g.IO.DeltaTime * 6.f);
    it_anim->second.cl2 = ImLerp(it_anim->second.cl2, selected ? menu[1] : 1.0f , g.IO.DeltaTime * 6.f);
    it_anim->second.cl3 = ImLerp(it_anim->second.cl3, selected ? menu[2] : 1.0f , g.IO.DeltaTime * 6.f);
    
    
    GetWindowDrawList()->AddText(NULL, 25.f, text_min, ImColor(it_anim->second.cl1, it_anim->second.cl2, it_anim->second.cl3, 1.0f), label);                              
    /*
    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
